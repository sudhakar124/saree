...
export default function SellerDashboard() {
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get('http://localhost:8000/seller/dashboard-stats', {
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post('http://localhost:8000/seller/products', { ...newProduct, image_url: imageUrl }, {
        headers: { Authorization: `Bearer ${token}` }
      });
